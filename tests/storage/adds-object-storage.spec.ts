...
test.describe('Backup storage', () => {
  ...
  for (const target of configured) {
    test(`Add ${target.label} object storage succeeds end-to-end`, async ({
      ...
      // A target's bucket can be reused by another spec file (see provider-switch-clears-the-fields.spec.ts
      // for DataCore) — the stale-bucket sweep and the backend's bucket-uniqueness constraint both make
      // concurrent access unsafe under the fullyParallel suite, so bucket-bearing targets serialize on it.
      if (target.provider === 'azure') {
        await run();
      } else {
        await withExclusiveBucket(target.bucket, run);
      }
    });
  }
